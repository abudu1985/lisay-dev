...
const QuoteBlock = () => {
  ...
  useEffect(() => {
    if (token) {
      ...
      setTime(left);
      if (left && left > 0) {
        sendQuoteRequest("/quotes", "GET", {}, token)
          .then((data) => {
  ...
  }, [isAuth, time, token, dispatch, toast]);

  const addQuote = (text) => {
    sendQuoteRequest("/quotes", "POST", { text }, token)
      .then((quote) => {
  ...
  const onEdit = (id, text) => {
    if (!id || !text) return;
    sendQuoteRequest(`/quotes/${id}`, "POST", { id, text }, token)
      .then((quote) => {
  ...
  const onDelete = (id) => {
    sendQuoteRequest(`/quotes/${id}`, "DELETE", {}, token)
      .then((data) => {
  ...
  const handleSubmit = (e) => {
    ...
    sendQuoteRequest("/users/login", "POST", credentials)
      .then(({ token }) => {
        setToken(token);
  ...
  return (
    <div className="quoteBlock">
      ...
      <QuoteList onEdit={onEdit} onDelete={onDelete} />
    </div>
  );
};
